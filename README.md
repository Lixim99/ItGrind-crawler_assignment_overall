...
- проверка `robots.txt`, дробный `Crawl-delay`, сопоставление User-Agent по product token и конкурентно-безопасное кэширование правил отдельно для каждого домена;
- очередь URL с приоритетом и ограничением глубины;
- фильтры домена, включения и исключения URL;
- защита от повторной обработки URL с канонизацией регистра host и стандартных портов;
