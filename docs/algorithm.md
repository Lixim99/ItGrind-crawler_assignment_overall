...
7. В блоке `finally` всегда освобождает оба семафора.

Таймаут каждой попытки состоит из `total`, `connect` и `sock_read`. Во время повторов `crawl()` умножает все три значения на номер попытки.
...
- каждый URL нормализуется, получает глубину `0` и добавляется в очередь.
...
3. загружает страницу через `RetryStrategy`;
4. парсит HTML;
...
`RetryStrategy` используется workers внутри `crawl()`. Прямые вызовы `fetch_url()` и `fetch_and_parse()` не выполняют автоматические повторы.
...
Для HTTP `429` задержка дополнительно умножается на два. После исчерпания `max_retries` исключение возвращается worker-у, URL записывается в `failed_urls`, а обработка очереди продолжается.
...
| `RobotsBlockedError` | запрет в robots.txt | Нет; URL учитывается как заблокированный |
...
Парсер `robots.txt` кэшируется на домен внутри одного запуска экземпляра `AsyncCrawler`. Ответ `4xx` при загрузке файла трактуется как разрешение обхода, а `5xx` или сетевая ошибка — как запрет обхода.
...
- `JSONStorage` — JSON Lines с буфером;
...
Буфер сбрасывается после накопления десяти записей и при `close()`. Ошибка записи повторяется до трёх раз с backoff. Если сохранение страницы окончательно не удалось, worker логирует ошибку и продолжает обход; результат парсинга всё равно остаётся в `processed_urls`.
...
Статистика экспортируется в JSON и HTML. Все внешние ресурсы освобождаются методом `close()`: закрывается `aiohttp.ClientSession`, сбрасывается буфер и закрывается выбранное хранилище.
