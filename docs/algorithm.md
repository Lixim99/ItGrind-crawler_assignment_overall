...
- схема и hostname приводятся к нижнему регистру;
...
Поэтому, например, `HTTPS://Example.com:443` и `https://example.com/`
попадают в очередь как один и тот же URL.
...
`RobotsParser` хранит отдельные правила и lock для каждого домена. Поэтому один экземпляр безопасно обслуживает несколько доменов, а конкурентные workers не дублируют загрузку одного `/robots.txt`. Для сопоставления правил из полного User-Agent выделяется product token: например, `MyBot/1.0` проверяется как `MyBot`. `Crawl-delay` разбирается отдельно и поддерживает как целые, так и дробные значения (`0.5`). Ответ `4xx` при загрузке файла трактуется как разрешение обхода, а `5xx` или сетевая ошибка — как запрет обхода.
